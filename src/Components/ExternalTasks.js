...
const ExternalTasks = () => {
  ...
  const fetchTasks = async () => {
    const querySnapshot = await getDocs(collection(db, "manualTasks"));
  ...
  const fetchCounter = async () => {
    const counterDoc = await getDoc(doc(db, "counters", "taskCounter2"));
    if (counterDoc.exists()) {
      return counterDoc.data().currentId;
    } else {
      await setDoc(doc(db, "counters", "taskCounter2"), { currentId: 0 });
      return 0;
    }
  };
  ...
  const incrementCounter = async () => {
    ...
    await setDoc(doc(db, "counters", "taskCounter2"), { currentId: newId });
    return newId;
  };
  ...
  const handleAddTask = async () => {
    try {
      ...
      const taskDoc = doc(db, "manualTasks", newId.toString());
  ...
  const handleUpdateTask = async () => {
    const taskDoc = doc(db, "manualTasks", currentTaskId.toString());
    try {
  ...
  const handleDeleteTask = async (id) => {
    try {
      const taskDoc = doc(db, "manualTasks", id.toString());
  ...
  return (
    <>
      {loading ? (
        ...
        <div
          id="refer"
          className="w-full flex flex-col h-[100vh] scroller overflow-y-auto pt-4 pb-[150px]"
        >
